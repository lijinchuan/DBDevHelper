...
namespace Biz.WCF
{
    public class WebSrvClient
    {
        private string _url = "";

        private Dictionary<string, List<InterfaceInfo>> _interfaceInfos;

        private WebSrvClient(string url)
        {
            _url = url;
            _interfaceInfos = new Dictionary<string, List<InterfaceInfo>>();
        }

        public static WebSrvClient CreateClient(string url)
        {
            return new WebSrvClient(url);
        }
                ...
                if (ele != null)
                {
                    foreach (XmlNode e in ele)
                    {
                        var p = new ParamInfo
                        {
                            ...
                            Type = e.Attributes["type"]==null?"object": TrimNameSpace(e.Attributes["type"].Value),
        ...
        public Dictionary<string, List<InterfaceInfo>> GetInterfaceInfos()
        {
            ...
            var resp = httpRequestEx.DoRequest($"{this._url}?WSDL", new byte[0], method: LJC.FrameWorkV3.Comm.WebRequestMethodEnum.GET);
            ...
            foreach (XmlNode bindnode in bindingnodes)
            {
                ...
                if (!binddic.ContainsKey(binding.Type))
                {
