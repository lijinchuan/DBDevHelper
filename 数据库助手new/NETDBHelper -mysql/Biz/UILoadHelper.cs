...
namespace Biz
{
    public static class UILoadHelper
    {
        ...
        private static void LoadColumns(Form parent, TreeNode tbNode, DBSource server, Func<TBColumn, string> gettip)
        {
            ...
            List<ForeignKey> foreignKeys = LocalCacheManager<List<ForeignKey>>.Find($"{tb.DBName}", () => MySQLHelper.GetForeignKeys(server, tb.DBName), 1);

            foreach (TBColumn col in MySQLHelper.GetColumns(server, tb.DBName, tb.TBName))
            {
