...
namespace NETDBHelper.SubForm
{
    public partial class UpSertDlg : SubBaseDlg
    {
        ...
        private void SetControls(bool reset=false)
        {
            ...
            var cols = MySQLHelper.GetColumns(_source, _table.DBName, _table.TBName).ToList();
        ...
        private void UpdateItem()
        {
            ...
            try
            {
                ...
                StringBuilder sb = new StringBuilder($"update [{_table.DBName}].[{_table.TBName}] set ");
                ...
                //记录日志
                //备份
                var sqlquery = $"select * from [{_table.DBName}].[{_table.TBName}] where {string.Join(" and ", conditionCols.Select(p => "[" + p + "]=@" + p))}";

                var datatable = MySQLHelper.ExecuteDBTable(_source, _table.DBName, sqlquery, @params.Where(p => conditionCols.Contains(p.ParameterName.TrimStart('@'))).Select(p => new MySqlParameter(p.ParameterName, p.Value)).ToArray());
                ...
                MySQLHelper.ExecuteNoQuery(_source, _table.DBName, sb.ToString(), @params.ToArray());
        ...
        private void NewAdd()
        {
            ...
            try
            {
                ...
                StringBuilder sb = new StringBuilder($"insert into [{_table.DBName}].[{_table.TBName}](");
                bool hasError = false;
                List<MySqlParameter> @params = new List<MySqlParameter>();
                foreach (Control ctl in ItemsPannel.Controls)
                {
