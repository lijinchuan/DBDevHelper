                                ...
                                if (datatableobject != null)
                                {
                                    ...
                                    foreach (var r in datatableobject.Rows)
                                    {
                                        ...
                                        if (table.Rows.Count >= batchSize)
                                        {
                                            ...
                                            try
                                            {
                                                MySQLHelper.SqlBulkCopy(connsqlserver.DBSource, datatableobject.DBName, (int)TimeOutMins.Value * 60 * 1000, "`" + datatableobject.DBName + "`.`" + datatableobject.TableName + "`", table);
                                            }
                                            catch (Exception ex)
                                            {
