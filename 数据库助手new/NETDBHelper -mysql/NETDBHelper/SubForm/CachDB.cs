                ...
                foreach (var item in CLBDBs.CheckedItems)
                {
                    try
                    {
                        ...
                        foreach (var tb in tbrows)
                        {
                            ...
                            var cols = MySQLHelper.GetColumns(DBSource, tbinfo.DBName, tbinfo.TBName).ToList();

                            //导出前100条语句
                            try
                            {
                                foreach (var data in MySQLHelper.ExportData2(cols, datatableobject, DBSource, tbinfo, (int)NUDMaxNumber.Value, () => cancel || stop))
                                {
                                    if (cancel)
                                    {
                                        break;
