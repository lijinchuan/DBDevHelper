            ...
            try
            {
                ...
                foreach (var item in BigEntityTableEngine.LocalEngine.List<WatchTaskInfo>(nameof(WatchTaskInfo), 1, int.MaxValue))
                {
                    ...
                    try
                    {
                        ...
                        var obj = Biz.Common.Data.OracleHelper.ExecuteScalar(item.DBServer, item.ConnDB, item.Sql);
                        ...
                        if (!StopTaskLoop)
                        {
                            LJC.FrameWorkV3.Data.EntityDataBase.BigEntityTableEngine.LocalEngine.Update<WatchTaskInfo>(nameof(WatchTaskInfo), item);
                    ...
                    {
                        LJC.FrameWorkV3.Data.EntityDataBase.BigEntityTableEngine.LocalEngine.Insert<WatchTaskLog>(nameof(WatchTaskLog), new WatchTaskLog
                        {
                            CDate = DateTime.Now,
                            TaskId = item.ID,
